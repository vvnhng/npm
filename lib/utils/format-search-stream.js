...
function normalizePackage (data, opts) {
  var nameTrim = ansiTrim(data.name)
  var descriptionTrim = ansiTrim(data.description)
  var authorTrim = data.maintainers.map((m) => `=${ansiTrim(m.username)}`).join(' ')
  var keywordsTrim = Array.isArray(data.keywords)
